...
- **The Contract Source of Truth:** The React frontend relies strictly on the backend documentation. Therefore, **Every single HTTP endpoint MUST be documented using Swagger/OpenAPI**.
...
- **CORS:** Configure CORS strictly to accept requests ONLY from the authorized React frontend domain (local dev origin, and the production domain once it is defined — TBD, not registered yet), with `credentials: true`.
